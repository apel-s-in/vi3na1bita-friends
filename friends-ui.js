...
export const mountFriendsUI = (root, core, { onGameInvite = null, onEnableWebPush = null, getUnread = null, getUnreadMeta = null, getWebPushEnabled = null, onUnreadClick = null, onChatOpened = null, onVoiceOpened = null } = {}) => {
  ...
  const openChatModal = (friendId, name = 'Друг') => {
    ...
    const input = ov.querySelector('textarea, input');
    ...
    const scrollToMessage = msgId => {
      const row = rowsByMsgId.get(msgId);
      if (!row) return false;
    ...
    const updateMessage = msg => {
      ...
      const nextMsgId = cur.msgId || prevMsgId || '';
      const nextClientId = cur.clientMsgId || prevClientId || '';
    ...
    const append = msg => {
      ...
      seen.add(key);
      if (msg.msgId) messagesById.set(msg.msgId, msg);
      if (msg.clientMsgId) messagesById.set(msg.clientMsgId, msg);
      ...
      row.dataset.clientMsgId = msg.clientMsgId || '';
      row.innerHTML = `
      ...
      row.querySelector('[data-reply-to]')?.addEventListener('click', e => {
        e.preventDefault();
        scrollToMessage(e.currentTarget.dataset.replyTo);
      });
      row.querySelector('.vf-chat-status-btn')?.addEventListener('click', () => showStatus(messagesById.get(msg.msgId || msg.clientMsgId) || msg));
      row.querySelector('.vf-chat-retry')?.addEventListener('click', () => retryMessage(msg));
      row.querySelector('.vf-chat-bubble')?.addEventListener('click', () => openMessageMenu(msg));

      if (msg.msgId) rowsByMsgId.set(msg.msgId, row);
      if (msg.clientMsgId) rowsByClientId.set(msg.clientMsgId, row);
      log.append(row);
    ...
    ov.querySelector('.vf-chat-reply-x').onclick = () => { replyTo = null; renderReply(); };
    ...
    activeChatApi = { friendId, pushIncomingMessage, scrollToMessage };
    load();
    scheduleLoad();
