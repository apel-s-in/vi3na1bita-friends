...
import { getPlayableGames } from './games-registry.js?v=9.2.4';
import { createFriendsModalAdapter } from './modal-adapter.js?v=9.2.4';
import { openTextChatModal } from './chat-text-ui.js?v=9.2.4';
import { openVoiceCallUi } from './voice-call-ui.js?v=9.2.4';
...
export const mountFriendsUI = (root, core, { onGameInvite = null, onEnableWebPush = null, getUnread = null, getWebPushEnabled = null, onUnreadClick = null, onChatOpened = null, onVoiceOpened = null } = {}) => {
  ...
  const renderList = (friends = [], presence = {}) => {
    const webPushEnabled = typeof getWebPushEnabled === 'function' && getWebPushEnabled();
    return `
  ...
  const bindList = () => {
    ...
    el.querySelector('[data-act="refresh"]')?.addEventListener('click', async event => {
  ...
  const modal = createFriendsModalAdapter({ openModal });
  ...
  const openAddModal = async () => {
    ...
    try {
      ...
      `;
      ov.querySelector('[data-x="copy"]')?.addEventListener('click', () => {
        ...
        try {
          ...
          if (val.includes('addFriend=')) {
            ...
            try {
              ...
              u = new URL('https://dummy.com/' + (val.startsWith('?') ? val : '?' + val));
            }
  ...
  return { refresh, openChat, openVoiceCall, getActiveChatFriendId: () => activeChatApi?.friendId || '', pushIncomingChat };
};
